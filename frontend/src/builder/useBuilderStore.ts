...
import type { BuilderState, BuilderAction, BlockType } from './types';
  ...
  switch (action.type) {
    case 'ADD_BLOCK': {
      ...
      if (action.parentId && state.blocks[action.parentId]) {
        ...
        blocks[action.parentId] = { ...parent, children: [...parent.children, id] };
        return { ...state, blocks, selectedId: id, pendingEditId };
      }
      return { ...state, blocks, rootIds: [...state.rootIds, id], selectedId: id, pendingEditId };
    }
    case 'DUPLICATE_BLOCK': {
      ...
      const parent = Object.values(state.blocks).find((b) => b.children.includes(action.id));
      if (parent) {
